...
**ProSoft** | Version 1.1.0 | May 2026

## **Getting Started**

Launch EasySave.exe from your terminal. If launching for the first time, you may be prompted to select your language (fr / en). The main menu will then appear automatically.
...
| 7 | **General settings (Log formats, etc.)** |
| 8 | Quit EasySave |
...
* **Job name** — a label to identify the backup (e.g. Backup\_Docs)  
...
## **General Settings (Option 7\)**

Select option 7 to access and modify your global application preferences.

Currently, this menu allows you to change the **Daily Log Format**:

Current log format: Xml  
Choose the log format \[1=JSON / 2=XML\]:

* Enter 1 to generate daily logs in JSON format.  
* Enter 2 to generate daily logs in XML format.

Your choice is saved immediately in the application settings (%ProgramData%\\EasySave\\settings.json) and applies to all subsequent backup jobs.
...
* **Daily log** — %ProgramData%\\EasySave\\logs\\yyyy-MM-dd.json (or .xml)  
  Records every file transfer: source, destination, size, transfer time, and errors. The format depends on your configuration (Menu Option 7).  
